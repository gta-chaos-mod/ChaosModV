...
namespace Memory
{
	...
	{
		Handle handle = Memory::FindPattern("?? 89 5C ?? ?? 57 ?? 83 EC ?? ?? 8D 3D ?? ?? ?? ?? ?? 8D ?? ?? ?? E8", "56 57 48 83 EC 48 E8 ?? ?? ?? ?? 89");
		if (!handle.IsValid())
			return nullptr;
	...
	inline void SetRadarParams(std::function<void(MinimapData &, MinimapData)> process)
	{
		...
		static auto minimapData = []() -> MinimapData *
		{
			Handle handle = Memory::FindPattern(
			    "?? 8D 15 ?? ?? ?? ?? ?? 6B C9 78 8B 44 ?? ?? 89 03 8B 44 ?? ?? 89 43 04 8A 4C ?? ??", "48 8D 3D ?? ?? ?? ?? F2 0F 10 44 3B 6C");

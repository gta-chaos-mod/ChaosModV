...
{
	m_ProxySecsBeforeVoting = g_optionsManager.GetTwitchValue<int>("TwitchVotingSecsBeforeVoting", OPTION_DEFAULT_TWITCH_SECS_BEFORE_VOTING);

	m_enableProxyChanceSystem = g_optionsManager.GetTwitchValue<bool>("TwitchVotingChanceSystem", OPTION_DEFAULT_TWITCH_PROPORTIONAL_VOTING);
	...
	m_enableProxyRandomEffectVoteable = g_optionsManager.GetTwitchValue<bool>("TwitchRandomEffectVoteableEnable", OPTION_DEFAULT_TWITCH_RANDOM_EFFECT);

	g_effectDispatcher->OverrideTimerDontDispatch(true);

	m_pipeHandle = CreateNamedPipe("\\\\.\\pipe\\ChaosModVProxyChatPipe", PIPE_ACCESS_DUPLEX, PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_NOWAIT,
...
void ProxyManager::Tick()
{
	...
	if (m_lastVotesFetchTime < curTick - 500)
	{
		...
		if (m_isVotingRunning && m_enableProxyChanceSystem && !m_enableProxyPollVoting)
		{
	...
	else if (!m_isVotingRunning && m_receivedFirstPing && (m_ProxySecsBeforeVoting == 0 || g_effectDispatcher->GetRemainingTimerTime() <= m_ProxySecsBeforeVoting) && m_isVotingRoundDone)
	{
		// New voting round
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (i == 3)
			{
				if (m_enableProxyRandomEffectVoteable)
				{
				...
				if (index <= addedUpWeight)
				{
					...
					targetChoice = std::make_unique<ChoosableEffect>(pair.first, effectData.HasCustomName ? effectData.CustomName : effectData.Name,
						!m_alternatedVotingRound
							? i + 1
							: m_enableProxyRandomEffectVoteable
								? i + 5
								: i + 4
					);
					break;
	...
	if (m_isVotingRunning && !m_noVoteRound && !m_enableProxyPollVoting)
	{
		// Print voteables on screen

		// Count total votes if chance system is enabled
		int totalVotes = 0;
		if (m_enableProxyChanceSystem)
		{
			for (const std::unique_ptr<ChoosableEffect>& choosableEffect : m_effectChoices)
			{
...
bool ProxyManager::HandleMsg(const std::string& msg)
{
	if (msg == "ping")
	{
...
void ProxyManager::SendToPipe(std::string&& msg)
{
	msg += "\n";
	WriteFile(m_pipeHandle, msg.c_str(), msg.length(), NULL, NULL);
}

void ProxyManager::ErrorOutWithMsg(const std::string&& msg)
{

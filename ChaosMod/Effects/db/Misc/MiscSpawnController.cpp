...
static void SpawnProp(const char* propName, Vector3 offsetPerInstance)
{
	...
	for (int i = 0; i < g_MetaInfo.m_fChaosMultiplier; i++)
	{
		...
		spawnPos = spawnPos + offsetPerInstance;
	}
}

static void OnStartUFO()
{
	SpawnProp("p_spinning_anus_s", Vector3(0.f, 0.f, 100.f));
}
...
{
	SpawnProp("prop_ld_ferris_wheel", Vector3(0.f, 50.f, 0.f));
}

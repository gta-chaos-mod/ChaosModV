...
static void OnTick()
{
	...
	if (!availablePropModels.empty())
	{
		...
		{
			if (DOES_ENTITY_EXIST(ped) && !IS_PED_A_PLAYER(ped) && pedPropsMap.count(ped) == 0 && !IS_PED_IN_ANY_VEHICLE(ped, false))
			{
		...
		{
			if (DOES_ENTITY_EXIST(veh) && !IS_PED_A_PLAYER(veh) && pedPropsMap.count(veh) == 0)
			{
				// Rather than using a predefined list of prop hashes, get a random existing prop and use that as the model
				Hash propModel = availablePropModels[g_Random.GetRandomInt(0, availablePropModels.size() - 1)];
				LoadModel(propModel);
				...
				SET_ENTITY_VISIBLE(veh, false, 0);

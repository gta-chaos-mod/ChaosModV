...
static void OnStart()
{
	...
	if (IS_PED_IN_ANY_VEHICLE(playerPed, false))
	{
		...
		if (seats >= 2)
		{
			SET_PED_INTO_VEHICLE(playerPed, veh, 0); // -1 = driver, 0 = passenger 0+ = other passenger seats
			auto jesus = CREATE_PED_INSIDE_VEHICLE(veh, 4, modelHash, -1, true, false);
			TASK_VEHICLE_DRIVE_WANDER(jesus, veh, 50, 1074528293); // 1074528293 = Rushed, 50 = 50 Meters per second = 111.847 MPH
		}
	}
}

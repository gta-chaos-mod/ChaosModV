...
static void OnStart()
{
	...
	for(int i = 0; i < 20; i++)
	{		
		Ped ped = CreatePoolPed(4, modelHash, playerPos.x + g_random.GetRandomInt(-5, 5), playerPos.y + g_random.GetRandomInt(-5, 5), playerPos.z, 0.f);
		...
		GIVE_WEAPON_TO_PED(ped, GET_HASH_KEY("WEAPON_BAT"), 0, true, true);

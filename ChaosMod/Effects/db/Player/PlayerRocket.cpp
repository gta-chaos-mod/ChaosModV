...
static void OnStart()
{
	...
	int launchTimer = LAUNCH_TIMER * g_MetaInfo.m_fChaosMultiplier;
	int beepTimer = LAUNCH_TIMER * g_MetaInfo.m_fChaosMultiplier;
	while (true)
	{
		...
		if ((launchTimer -= curTimestamp - lastTimestamp) < beepTimer)
		{
			...
			SET_ENTITY_VELOCITY(playerPed, 0.f, 0.f, 5.f * g_MetaInfo.m_fChaosMultiplier);
		}
		...
		if (launchTimer <= 0)
		{
			...
			SET_ENTITY_VELOCITY(playerPed, 0.f, 0.f, 100.f * g_MetaInfo.m_fChaosMultiplier);	// Increasing the z value doesn't seem to affect the force anymore while ragdolling. 

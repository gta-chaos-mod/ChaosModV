...
static void OnTick()
{
	...
	if (IS_PED_WEAPON_READY_TO_SHOOT(playerPed))
	{
		...
		for (Ped ped : GetAllPeds())
		{
			...
			if (ped != playerPed && IS_TRACKED_PED_VISIBLE(ped) && !IS_PED_DEAD_OR_DYING(ped, true) && IS_PED_HUMAN(ped)
			    && !IS_PED_A_PLAYER(ped) && IS_TRACKED_PED_VISIBLE(ped))
			{
				Vector3 pedCoord = GET_ENTITY_COORDS(ped, 0);
				float distance   = GET_DISTANCE_BETWEEN_COORDS(playerCoord.x, playerCoord.y, playerCoord.z, pedCoord.x,
				                                               pedCoord.y, pedCoord.z, true);

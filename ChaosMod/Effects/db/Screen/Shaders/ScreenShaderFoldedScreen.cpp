...
static void OnStart()
{
	...
	{
	case 0:
		shaderSrc = "return HDRSampler.Sample(g_samLinear, float2(lerp(texcoord.x >= 0.5, texcoord.x < 0.5, "
	...
	if (!shaderSrc.empty())
	{
		Hooks::OverrideShader(EOverrideShaderType::LensDistortion, ms_szShaderSrcPrefix + shaderSrc);
	}
}

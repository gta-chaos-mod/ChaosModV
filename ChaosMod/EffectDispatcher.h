...
public:
	...
	void Reset();
	void ResetTimer();
	void OverrideEffectName(EffectType& effectType, std::string& overrideName);
...
private:
	...
	{
	public:
		ActiveEffect(const EffectIdentifier& effectIdentifier, RegisteredEffect* registeredEffect, const std::string& name, const std::string& fakeName, float timer) : EffectIdentifier(effectIdentifier), RegisteredEffect(registeredEffect),
			Name(name), FakeName(fakeName), Timer(timer), MaxTime(Timer)
		{

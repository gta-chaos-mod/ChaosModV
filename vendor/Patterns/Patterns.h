...
inline void set_base(uintptr_t address) {
  ...
  // pointer-style cast to ensure unsigned overflow ends up copied directly into
  // a signed value
...
private:
  ...
  inline pattern(uintptr_t module) : pattern(module, 0) {}

  inline pattern(uintptr_t begin, uintptr_t end)
      : m_rangeStart(begin), m_rangeEnd(end) {}

public:
  pattern(std::string_view pattern) : hook::pattern(getRVA(0)) {
...
inline pattern make_range_pattern(uintptr_t begin, uintptr_t end,

...
void set_base() { set_base((uintptr_t)GetModuleHandle(nullptr)); }
...
static void TransformPattern(std::string_view pattern,
...
public:
  ...
  explicit executable_meta(uintptr_t module) : m_begin(module) {
...
#endif
  {
    ...
    if (range.first != range.second) {
      std::for_each(range.first, range.second,
                    [&](const auto &hint) { ConsiderHint(hint.second); });
...
void pattern::EnsureMatches(uint32_t maxCount) {
  ...
  std::fill(std::begin(Last), std::end(Last),
            lastWild == std::string::npos ? -1
                                          : static_cast<ptrdiff_t>(lastWild));
  ...
  for (uintptr_t i = executable.begin(), end = executable.end() - maskSize;
       i <= end;) {
...
#ifdef PATTERNS_CAN_SERIALIZE_HINTS
  const uint8_t *pattern = m_bytes.data();
  const uint8_t *mask = m_mask.data();
...
#if defined(PATTERNS_USE_HINTS) && defined(PATTERNS_CAN_SERIALIZE_HINTS)
